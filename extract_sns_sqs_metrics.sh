...
list_sns_topics() {
    ...
    jq -r --arg substring "$substring" '.Topics[] | select(.TopicArn | contains($substring) and (contains("dlq") | not) and (contains("reduce") | not)) | .TopicArn'
}
...
list_sqs_queues() {
    ...
    jq -r --arg substring "$substring" '.QueueUrls[] | select(. | contains($substring) and (contains("dlq") | not) and (contains("reduce") | not))'
}

...
public class S3BenchmarkMessageProducer implements MessageProducer {
    private static final Logger log = LoggerFactory.getLogger(S3BenchmarkMessageProducer.class);
    private final UniformRateLimiter rateLimiter;
    private Supplier<Long> nanoClock;

    S3BenchmarkMessageProducer(UniformRateLimiter rateLimiter) {
        this(System::nanoTime, rateLimiter);
    }

    S3BenchmarkMessageProducer(Supplier<Long> nanoClock, UniformRateLimiter rateLimiter) {
